 ...
 * @param {Object[]} movies - An array of movies. See the `movies.js` file for an example of this array.
 * @returns {Object} An object where keys are movie ratings (e.g. "PG") 
 * and the values are how many movies in the array have that rating (e.g. 7).
 *
 * EXAMPLE:
...
function filterByGenre(movies, genre) {
  let arr = []
for (let type of movies){
  if (type.genre.toLowerCase().includes(genre.toLowerCase())){
arr.push(type)
  }
}
return arr
...
function getAllMoviesReleasedAtOrBeforeYear(movies, year) {
  let arr = []
  for(let date of movies){
    let release = date.released.split(" ")
if (Number(release[2]) <= year){
arr.push(date)
}
  }
  return arr
...
function getBiggestBoxOfficeMovie(movies) {
  ...
  let accumulator = 0
  for(let movie of movies){
let string = movie.boxOffice.replace(",", "")
